...
{
	DBGPRINT(ELIAN_DBG_WARN,("[MSC] done: AM=%d, ssid=%s, pwd=%s, user=%s, cust_data_len=%d, cust_data=%s,\n",
...
{
	DBGPRINT(ELIAN_DBG_WARN,("MSC msc_proc_create: %p\n", priv));
	if(entry == NULL)
	{
...
{
	DBGPRINT(ELIAN_DBG_WARN,("MSC msc_proc_destroy: %s\n", MSC_PROC_NAME));
	if(entry != NULL)
	{
