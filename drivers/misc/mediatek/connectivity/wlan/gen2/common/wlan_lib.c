...
#if CFG_SUPPORT_FIXED_MAC
		fixedMacStatus = kalCfgFixedMacRead(prAdapter->prGlueInfo, 0, rMacAddr);
		if ((fixedMacStatus == FALSE)
		|| (EQUAL_MAC_ADDR(aucZeroMacAddr, rMacAddr))
...
#if CFG_SUPPORT_FIXED_MAC
		kalCfgFixedMacWrite(prAdapter->prGlueInfo, 0, rMacAddr);
		}
#endif
