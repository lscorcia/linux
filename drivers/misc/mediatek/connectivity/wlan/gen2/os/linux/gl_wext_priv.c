...
/*----------------------------------------------------------------------------*/
int
priv_msc_elian(IN struct net_device *prNetDev,
		IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN OUT char *pcExtra)
{
	char result[256] = {0};
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	ASSERT(prIwReqData);
	if (!prNetDev || !prIwReqData) {
		DBGLOG(REQ, INFO, "priv_msc_elian(): invalid param(0x%p, 0x%p)\n", prNetDev, prIwReqData);
		return -EINVAL;
	}
	...
	ASSERT(prGlueInfo);
	if (!prGlueInfo) {
		DBGLOG(REQ, INFO, "priv_msc_elian(): invalid prGlueInfo(0x%p, 0x%p)\n",
				   prNetDev, *((P_GLUE_INFO_T *) netdev_priv(prNetDev)));
		return -EINVAL;
	}
	...
	if (pcExtra) {
		DBGLOG(REQ, WARN, "ELIAN CMD = %s\n", pcExtra);
		DBGLOG(REQ, INFO, "%s prIwReqData->data.length = %d\n", __func__, prIwReqData->data.length);
		msc_cmd_handler(pcExtra, prIwReqData->data.length, result, prGlueInfo->prAdapter);

		prIwReqData->data.length = strlen(result);
		strncpy(pcExtra, result, prIwReqData->data.length);
		DBGLOG(REQ, WARN, "ELIAN RETURN = %s\n", pcExtra);
	}
