...
#if CFG_SUPPORT_FIXED_MAC
#define WIFI_FIXED_MAC_FILE_NAME "/etc/WIFI_MAC"
#endif
/*******************************************************************************
*                             D A T A   T Y P E S
********************************************************************************
...
static int nvram_write(char *filename, char *buf, ssize_t len, int offset)
{
	...
	for(i = 0; i < 512; i++) {
		//if(sizeof(char)==read(fd,&buf_check,sizeof(char))) {
...
#if CFG_SUPPORT_FIXED_MAC
static int fixed_mac_read(char *filename, char *buf, ssize_t len, int offset)
{
	...
	if (IS_ERR(fd)) {
		DBGLOG(INIT, INFO, "[fixed_mac_read] : failed to open!!\n");
		return -1;
	}

	do {
		if (fd->f_op == NULL) {
			DBGLOG(INIT, INFO, "[fixed_mac_read] : f_op is NULL!!\n");
			break;
		}
		...
		if (fd->f_pos != offset) {
			if (fd->f_op->llseek) {
				if (fd->f_op->llseek(fd, offset, 0) != offset) {
					DBGLOG(INIT, INFO, "[fixed_mac_read] : failed to seek!!\n");
					break;
				}
			} else {
				fd->f_pos = offset;
		...
		retLen = __vfs_read(fd, p, len, &pos);
		if (retLen < 0)
			DBGLOG(INIT, ERROR, "[fixed_mac_read] : read failed!! Error code: %d\n", retLen);
...
static int fixed_mac_write(char *filename, char *buf, ssize_t len, int offset)
{
	struct file *fd;
	int retLen = -1;
	loff_t pos;
	...
	if (IS_ERR(fd)) {
		DBGLOG(INIT, INFO, "[fixed_mac_write] : failed to open!!\n");
		return -1;
	}

	do {
		if (fd->f_op == NULL) {
			DBGLOG(INIT, INFO, "[fixed_mac_write] : f_op is NULL!!\n");
			break;
		}
		/* End of if */
		if (fd->f_pos != offset) {
			if (fd->f_op->llseek) {
				if (fd->f_op->llseek(fd, offset, 0) != offset) {
					DBGLOG(INIT, INFO, "[fixed_mac_write] : failed to seek!!\n");
					break;
				}
			} else {
				fd->f_pos = offset;
		...
		retLen = __vfs_write(fd, p, len, &pos);
		if (retLen < 0)
			DBGLOG(INIT, ERROR, "[fixed_mac_write] : write failed!! Error code: %d\n", retLen);
...
BOOLEAN kalCfgFixedMacRead(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4Offset, OUT PUINT_8 mac)
{
	if (mac == NULL)
		return FALSE;

	if (fixed_mac_read(WIFI_FIXED_MAC_FILE_NAME,
		       (char *)mac, 6, u4Offset) != 6) {
		return FALSE;
	} else {
		return TRUE;
	}
...
BOOLEAN kalCfgFixedMacWrite(IN P_GLUE_INFO_T prGlueInfo, UINT_32 u4Offset, PUINT_8 mac)
{
	if (fixed_mac_write(WIFI_FIXED_MAC_FILE_NAME,
			(char *)mac, 6, u4Offset) != 6) {
		return FALSE;
	} else {
		return TRUE;
