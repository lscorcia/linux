				       ...
				       struct clk_rate_request *req)
{
	...
	dev_err(hdmi_phy->dev, "*** LUCA %s ENTER: req_rate: %lu Hz, best_parent_rate: %lu Hz\n", __func__, req->rate, req->best_parent_rate);
	dev_err(hdmi_phy->dev, "*** LUCA %s BEFORE: pll_rate = %lu Hz\n", __func__, hdmi_phy->pll_rate);

	hdmi_phy->pll_rate = req->rate;

	dev_err(hdmi_phy->dev, "*** LUCA %s AFTER: pll_rate = %lu Hz, best_parent_rate: %lu Hz\n", __func__,
					      ...
					      unsigned long parent_rate)
{
	...
	dev_err(hdmi_phy->dev, "*** LUCA %s ENTER parent_rate = %lu Hz\n", __func__,
		parent_rate);
	//dump_stack_lvl(KERN_ERR);
/*
	...
	dev_err(hdmi_phy->dev, "%s RG_HTPLL_PREDIV = %lu\n", __func__,
		val);

	val = FIELD_GET(RG_HTPLL_FBKDIV, tmp);
	out_rate *= (val + 1) * 2;

	dev_err(hdmi_phy->dev, "%s RG_HTPLL_FBKDIV = %lu\n", __func__,
		val);
	...
	dev_err(hdmi_phy->dev, "%s RG_HDMITX_TX_POSDIV = %lu, RG_HDMITX_EN_TX_POSDIV = %d\n", __func__,
		val, tmp & RG_HDMITX_EN_TX_POSDIV);

	if (tmp & RG_HDMITX_EN_TX_POSDIV)
		out_rate /= 5;

	dev_err(hdmi_phy->dev, "%s EXIT parent_rate = %lu Hz, out_rate = %lu Hz\n", __func__,
		parent_rate, out_rate);
