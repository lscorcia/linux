...
static int mt8167_codec_ul_rate_to_val(struct snd_soc_component *codec, int rate)
{
	int i;
...
static int mt8167_codec_dl_rate_to_val(struct snd_soc_component *codec, int rate)
{
	int i;
...
static int mt8167_codec_valid_new_rate(struct snd_soc_component *codec)
{
	...
	if (snd_soc_component_read(codec, ABB_AFE_CON11) &
			ABB_AFE_CON11_TOP_CTRL_STATUS)
		snd_soc_component_update_bits(codec, ABB_AFE_CON11,
...
static int mt8167_codec_setup_ul_rate(struct snd_soc_component *codec, int rate)
{
	uint32_t val = 0;
	...
	snd_soc_component_update_bits(codec, ABB_AFE_CON1, (0xF << 4), (val << 4));
	mt8167_codec_valid_new_rate(codec);

	return 0;
}

static int mt8167_codec_setup_dl_rate(struct snd_soc_component *codec, int rate)
{
	uint32_t val = 0;
	...
	snd_soc_component_update_bits(codec, ABB_AFE_CON1, GENMASK(3, 0), val);
	mt8167_codec_valid_new_rate(codec);
			...
			struct snd_soc_dai *codec_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		snd_soc_component_update_bits(codec, ABB_AFE_CON10, 0x1, 0x1);
...
static int mt8167_codec_aif_tx_from_dmic(struct snd_soc_component *codec)
{
...
static int mt8167_codec_aif_tx_from_amic(struct snd_soc_component *codec)
{
...
static int mt8167_codec_aif_tx_from_aif_rx(struct snd_soc_component *codec)
{
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, ABB_AFE_CON0, BIT(1), BIT(1));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(codec, ABB_AFE_CON0, BIT(1), 0x0);
		break;
	default:
		break;
	}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, ABB_AFE_CON0, 0x1, 0x1);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(codec, ABB_AFE_CON0, 0x1, 0x0);
		break;
	default:
		break;
	}
...
static void mt8167_codec_hp_depop_setup(struct snd_soc_component *codec)
{
	struct mt8167_codec_priv *codec_data = snd_soc_component_get_drvdata(codec);

	/* Set audio DAC bias current */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON01, (0x1F << 6), 0x0);
	/* Set the charge option of depop VCM gen. to "charge type" */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(18), BIT(18));
	/*
	...
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(21), 0x0);
	/* Enable the depop VCM generator. */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(22), BIT(22));
	/* Set the series resistor of depop mux of HP drivers to 62.5 Ohm. */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, (0x3 << 23), (0x3 << 23));
	/* Disable audio DAC clock. */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(27), 0x0);
	/* Enable the depop mux of HP drivers. */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(25), BIT(25));
}

static void mt8167_codec_hp_depop_cleanup(struct snd_soc_component *codec)
{
	/* Set the charge option of depop VCM gen. to "dis-charge type" */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(18), 0x0);
	/* Set the 33uF cap current step of depop VCM gen to dis-charge */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, (0x2 << 19), (0x2 << 19));
	/* Disable the depop VCM generator */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(22), 0x0);
}
			...
			struct mt8167_codec_priv *codec_data)
{
	...
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(28), 0x0);
}
			...
			struct mt8167_codec_priv *codec_data)
{
	...
	/* restore gain */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON01, GENMASK(2, 0),
			codec_data->pga_gain[HP_L_PGA_GAIN]);
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON01, GENMASK(5, 3),
			(codec_data->pga_gain[HP_R_PGA_GAIN]) << 3);

	/* HP Pre-charge function release */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(28), BIT(28));
	...
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(25), 0x0);
...
static int module_reg_read(struct snd_soc_component *codec, unsigned int reg, unsigned int *val,
...
static int module_reg_write(struct snd_soc_component *codec, unsigned int reg, unsigned int val,
	enum regmap_module_id id, unsigned int offset)
{
	struct mt8167_codec_priv *codec_data = snd_soc_component_get_drvdata(codec);
	int ret = 0;
...
// regmap functions
static unsigned int codec_reg_read(struct snd_soc_component *codec,
		unsigned int reg)
{
	...
	ret = module_reg_read(codec, reg, &val, id, offset);
	if (ret) {
		dev_err(codec->dev, "%s failed to read reg %x, ret = %d\n",
			__func__, reg, ret);
		return -1;
	}
...
static int codec_reg_write(struct snd_soc_component *codec,
			unsigned int reg, unsigned int val)
{
	...
	return module_reg_write(codec, reg, val,
			id, offset);
}
	...
	struct mt8167_codec_priv *codec_data)
{
	...
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON00, BIT(17), 0x0);
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON01, BIT(31), 0x0);

	/* Audio buffer quasi-current  */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, GENMASK(31, 30), 0x0);

	/* setup default gain */
	/* +4dB for voice buf gain */
	codec_data->pga_gain[LOUT_PGA_GAIN] = 0xB;
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, GENMASK(12, 9),
		(codec_data->pga_gain[LOUT_PGA_GAIN]) << 9);
