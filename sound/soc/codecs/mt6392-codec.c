...
static int mt6392_int_spk_turn_on(struct snd_soc_component *codec)
{
	struct mt6392_codec_priv *codec_data =
			snd_soc_component_get_drvdata(codec);
	int ret = 0;
...
static int mt6392_int_spk_turn_off(struct snd_soc_component *codec)
{
	struct mt6392_codec_priv *codec_data =
			snd_soc_component_get_drvdata(codec);
	int ret = 0;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	long int_spk_amp_en = snd_soc_component_read(codec, SPK_CON0) & BIT(0);
...
static int mt6392_codec_probe(struct snd_soc_component *codec)
{
	struct mt6392_codec_priv *codec_data =
			snd_soc_component_get_drvdata(codec);
	int ret = 0;
...
static void mt6392_codec_remove(struct snd_soc_component *codec)
{
#ifdef CONFIG_DEBUG_FS
	struct mt6392_codec_priv *codec_data =
			snd_soc_component_get_drvdata(codec);
...
static struct snd_soc_component_driver mt6392_codec_driver = {
