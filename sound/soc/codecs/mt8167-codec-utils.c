...
static uint32_t mt8167_codec_get_hp_cap_index(struct snd_soc_component *codec)
{
	return (snd_soc_component_read(codec, AUDIO_CODEC_CON02) &
			GENMASK(20, 19)) >> 19;
}
...
int mt8167_codec_valid_new_dc_comp(struct snd_soc_component *codec)
{
	/* toggle DC status */
...
void mt8167_codec_update_dc_comp(struct snd_soc_component *codec,
...
static bool mt8167_codec_ramp_one_step(struct snd_soc_component *codec,
...
void mt8167_codec_dc_offset_ramp(struct snd_soc_component *codec,
...
static int32_t mt8167_codec_query_avg_voltage(struct snd_soc_component *codec,
...
void mt8167_codec_show_voltage_of_hp(struct snd_soc_component *codec)
{
	int32_t voltage = 0;

	/* enable ADA_HPLO_TO_AUXADC */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(15), BIT(15));

	/* enable ADA_HPRO_TO_AUXADC */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(14), BIT(14));
	...
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(15), 0x0);

	/* disable ADA_HPRO_TO_AUXADC */
	snd_soc_component_update_bits(codec, AUDIO_CODEC_CON02, BIT(14), 0x0);
}
...
static uint64_t mt8167_codec_get_hp_cali_val(struct snd_soc_component *codec,
	uint32_t auxadc_channel,
...
void mt8167_codec_gather_comp_val(struct snd_soc_component *codec,
...
uint64_t mt8167_codec_get_hp_cali_comp_val(struct snd_soc_component *codec,
	uint32_t dccomp_val[HP_CALI_NUMS][HP_PGA_GAIN_NUMS],
	long dc_offset[HP_CALI_NUMS][HP_PGA_GAIN_NUMS],
	enum hp_channel_id cid)
{
