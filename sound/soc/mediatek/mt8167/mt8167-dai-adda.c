...
static int mt8167_afe_setup_i2s(struct mtk_base_afe *afe,
...
static int mt8167_afe_setup_adda_dl(struct mtk_base_afe *afe, unsigned int rate)
{
...
static int mt8167_afe_setup_adda_ul(struct mtk_base_afe *afe, unsigned int rate)
{
	unsigned int val = 0;
	unsigned int val2 = 0;
...
static void mt8167_afe_adda_shutdown(struct snd_pcm_substream *substream,
...
static int mt8167_afe_adda_hw_params(struct snd_pcm_substream *substream,
...
static int mt8167_afe_adda_prepare(struct snd_pcm_substream *substream,
				   struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = mt8167_afe_setup_adda_dl(afe, rate);
		if (ret)
			return ret;

		ret = mt8167_afe_setup_i2s(afe, substream, rate, bit_width);
		if (ret)
			return ret;

		regmap_update_bits(afe->regmap, AFE_ADDA_DL_SRC2_CON0, 1, 1);
	} else {
		ret = mt8167_afe_setup_adda_ul(afe, rate);
		if (ret)
			return ret;
...
static const struct snd_soc_dai_ops mt8167_afe_adda_ops = {
	.shutdown	= mt8167_afe_adda_shutdown,
	.hw_params	= mt8167_afe_adda_hw_params,
	.prepare	= mt8167_afe_adda_prepare,
};
...
static const struct snd_soc_dapm_widget mtk_dai_adda_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY_S("INT ADDA Enable", SUPPLY_SEQ_ADDA_AFE_ON,
			      AFE_DAC_CON0, 0, 0,
			      NULL, 0),
...
static const struct snd_soc_dapm_route mtk_dai_adda_routes[] = {
	...
	/* enable */
	{"INT ADDA Playback", NULL, "INT ADDA Enable"},
	{"INT ADDA Capture", NULL, "INT ADDA Enable"},

	/* clock */
	{"INT ADDA Playback", NULL, "aud_dac"},
	{"INT ADDA Playback", NULL, "aud_dac_predis"},
	{"INT ADDA Playback", NULL, "audio"},

	{"INT ADDA Capture", NULL, "audio"},
	{"INT ADDA Capture", NULL, "aud_adc"},
};
