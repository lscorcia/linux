...
static int mt8167_afe_i2s_fs(struct snd_pcm_substream *substream,
...
static int mt8167_afe_irq_fs(struct snd_pcm_substream *substream,
			     unsigned int rate)
{
	return mt8167_afe_i2s_fs(substream, rate);
}
...
static const struct snd_soc_dapm_widget mt8167_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt8167_memif_routes[] = {
	/* downlink */
	{"I05", NULL, "DL1"},
	{"I06", NULL, "DL1"},
	{"I07", NULL, "DL2"},
...
static struct mtk_base_irq_data mt8167_irq_data[MT8167_AFE_IRQ_NUM] = {
	[MT8167_AFE_IRQ_1] = {
		.id = MT8167_AFE_IRQ_1,
...
static struct mtk_base_afe_irq mt8167_irqs[MT8167_AFE_IRQ_NUM] = {
	{ .irq_data = &mt8167_irq_data[MT8167_AFE_IRQ_1] },
	{ .irq_data = &mt8167_irq_data[MT8167_AFE_IRQ_2] },
	{ .irq_data = &mt8167_irq_data[MT8167_AFE_IRQ_5] },
	{ .irq_data = &mt8167_irq_data[MT8167_AFE_IRQ_7] },
	{ .irq_data = &mt8167_irq_data[MT8167_AFE_IRQ_10] },
	{ .irq_data = &mt8167_irq_data[MT8167_AFE_IRQ_13] },
};

static struct mtk_base_memif_data mt8167_memif_data[MT8167_AFE_MEMIF_NUM] = {
	[MT8167_AFE_MEMIF_DL1] = {
		.name = "DL1",
...
struct mtk_base_afe_memif mt8167_memif[] = {
	[MT8167_AFE_MEMIF_DL1] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_DL1],
		.irq_usage = MT8167_AFE_IRQ_1,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_DL2] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_DL2],
		.irq_usage = MT8167_AFE_IRQ_7,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_VUL] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_VUL],
		.irq_usage = MT8167_AFE_IRQ_2,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_DAI] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_DAI],
		.irq_usage = MT8167_AFE_IRQ_2,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_AWB] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_AWB],
		.irq_usage = MT8167_AFE_IRQ_2,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_MOD_DAI] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_MOD_DAI],
		.irq_usage = MT8167_AFE_IRQ_2,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_HDMI] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_HDMI],
		.irq_usage = MT8167_AFE_IRQ_5,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_TDM_IN] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_TDM_IN],
		.irq_usage = MT8167_AFE_IRQ_10,
		.const_irq = 1,
	},
	[MT8167_AFE_MEMIF_MULTILINE_IN] = {
		.data = &mt8167_memif_data[MT8167_AFE_MEMIF_MULTILINE_IN],
		.data = &mt8167_memif_data[8],
		.irq_usage = MT8167_AFE_IRQ_13,
		.const_irq = 1,
	},
};
...
static struct snd_soc_dai_driver mt8167_memif_dai_driver[] = {
...
static int mt8167_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt8167_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt8167_memif_dai_driver);

	dai->dapm_widgets = mt8167_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt8167_memif_widgets);
	dai->dapm_routes = mt8167_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt8167_memif_routes);
...
static const dai_register_cb dai_register_cbs[] = {
	mt8167_dai_adda_register,
	mt8167_dai_memif_register,
};

static int mt8167_afe_component_probe(struct snd_soc_component *component)
{
...
static int mt8167_afe_runtime_suspend(struct device *dev)
{
	...
	regmap_update_bits(afe->regmap, AUDIO_TOP_CON0,
 	...
 	return mt8167_afe_disable_clock(afe);
}
...
static int mt8167_afe_runtime_resume(struct device *dev)
{
	...
	ret = mt8167_afe_enable_clock(afe);
	if (ret)
		return ret;

	// enable AFE clk
	ret = regmap_update_bits(afe->regmap, AUDIO_TOP_CON0, AUD_TCON0_PDN_AFE, 0);
	if (ret)
		return ret;
	...
	// unmask all IRQs
	ret = regmap_update_bits(afe->regmap, AFE_IRQ_MCU_EN, 0xff, 0xff);
	if (ret)
		return ret;

	// enable AFE
	ret = regmap_update_bits(afe->regmap, AFE_DAC_CON0, 0x1, 0x1);
	if (ret)
		return ret;
...
static const struct snd_soc_component_driver mt8167_afe_component = {
	.name = AFE_PCM_NAME,
	.pcm_new = mtk_afe_pcm_new,
	.pointer = mtk_afe_pcm_pointer,
	.probe = mt8167_afe_component_probe,
...
static int mt8167_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	struct mt8167_afe_private *afe_priv;
	...
	if (IS_ERR(afe->regmap)) {
		dev_err(dev, "could not get regmap from parent %s\n", dev->parent->init_name);
		ret = PTR_ERR(afe->regmap);
		goto err_pm_disable;
	}

	ret = regmap_attach_dev(dev, afe->regmap, &mt8167_afe_regmap_config);
	if (ret) {
	...
	afe->reg_back_up_list = &mt8167_afe_backup_list[0];
	afe->reg_back_up_list_num = ARRAY_SIZE(mt8167_afe_backup_list);
	...
	if (ret) {
		dev_warn(dev, "Failed to combine sub-dais, ret %d\n", ret);
		return ret;
	}

	afe->mtk_afe_hardware = &mt8167_afe_hardware;

	afe->irqs = mt8167_irqs;
	afe->irq_fs = mt8167_afe_irq_fs;

	afe->memif = &mt8167_memif[0];
	afe->memif_size = ARRAY_SIZE(mt8167_memif);
	afe->memif_fs = mt8167_afe_i2s_fs;

	afe->runtime_resume = mt8167_afe_runtime_resume;
	afe->runtime_suspend = mt8167_afe_runtime_suspend;

	ret = devm_snd_soc_register_component(dev,
					      &mt8167_afe_component,
					      afe->dai_drivers,
					      afe->num_dai_drivers);
	if (ret) {
	...
	if (!irq_id) {
		dev_err(dev, "np %s no irq\n", np->name);
		ret = -ENXIO;
		goto err_pm_disable;
	}

	ret = devm_request_irq(dev, irq_id, mt8167_afe_irq_handler,
...
static void mt8167_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;

	pm_runtime_disable(dev);
	if (!pm_runtime_status_suspended(dev))
		mt8167_afe_runtime_suspend(dev);
}
...
static const struct dev_pm_ops mt8167_afe_pm_ops = {
	RUNTIME_PM_OPS(mt8167_afe_runtime_suspend,
		       mt8167_afe_runtime_resume, NULL)
};
